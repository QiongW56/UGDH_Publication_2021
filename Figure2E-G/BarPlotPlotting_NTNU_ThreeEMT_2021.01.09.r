...
library(readxl)
datD <- data.frame(read_excel("2 NTNU_Proteomics_D492 vs. D492M_28.12.2018.xlsx"))
datH <- data.frame(read_excel("2 NTNU_Proteomics_HMLE vs. HMLE_M_28.12.2018.xlsx"))
datP <- data.frame(read_excel("2 NTNU_Proteomics_PMC42_LA vs. PMC42_ET_28.12.2018.xlsx"))

funDat <- function(dat, GN, EMTmodel){
  if (EMTmodel == "D492"){
  ...
  colnames(dat)[2:7] <- substr(colnames(dat)[2:7], 12, nchar(colnames(dat)[2:7]))
...
funGN <- function(GN){
  datD_GN <- funDat(datD, GN, "D492")
  datH_GN <- funDat(datH, GN, "HMLE")
  datP_GN <- funDat(datP, GN, "PMC42")
  ...
  ggplot.dat <- function(dat){
    ...
    file <- paste(format(Sys.time(), "%F %H-%M-%S"), colnames(dat)[4], "ThreeEMT_bar.plot.tiff", sep = " ")
