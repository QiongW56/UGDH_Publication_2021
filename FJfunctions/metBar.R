...
metBar <- function(dataset,group,fillgroup,met,colors, upperError = FALSE, normby = NULL, normsample = NULL)
{
  bCol <- TRUE
  if (missing(colors)){
    bCol <- FALSE
  ...
  if (!is.null(normby)){
    if (normby != fillgroup & normby != group){
      ...
      normF <- mean(dataset[dataset[,normidx] == "Baseline",metidx])
      dataset[,metidx] <- dataset[,metidx]/normF
    }
