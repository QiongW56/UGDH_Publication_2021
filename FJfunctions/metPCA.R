...
#--------ATH------------
# To do:
# Create a function that takes raw data set, calculates and plots
# the first two PCs, with scree plot and loading plot.
...
p <- p +  geom_point(aes(fill = ggdata[,gro]),shape = 21, size = pointsize)+
         # scale_x_reverse(name = PC1lab)+ #--- to flip the x axis
          scale_x_continuous(name = PC1lab) +
          scale_y_continuous(name = PC2lab)+
